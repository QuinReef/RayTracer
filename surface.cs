...
namespace Template
{
	public class Surface
	{
		...
		int TranslateX(double desiredX)
		{
			int size = 16;
			float step = width / size;
			return (int)((width - 1) / 2 + desiredX * step);
		}
		int TranslateY(double desiredY)
		{
			int size = 9;
			float step = height / size;
			return (int)(height - 1 - desiredY * step);
		}
		public void DrawCircle(float xCent, float zCent, float rad, int c)
		{
			int lines = 100;
			double pointOnCircleAx = (xCent + (rad * Math.Cos(0 * Math.PI * 2 / lines)));
			double pointOnCircleAz = (zCent + (rad * Math.Sin(0 * Math.PI * 2 / lines)));

			for (int a = 1; a <= lines; a++)
			{
				double pointOnCircleBx = (xCent + (rad * Math.Cos(a * Math.PI * 2 / lines)));
				double pointOnCircleBz = (zCent + (rad * Math.Sin(a * Math.PI * 2 / lines)));
				//s.Plot((int)(pointOnCircleX*s.width),(int)(pointOnCircleZ*s.height), c);

				Line(TranslateX(pointOnCircleAx), TranslateY(pointOnCircleAz), TranslateX(pointOnCircleBx), TranslateY(pointOnCircleBz), c);

				pointOnCircleAx = pointOnCircleBx;
				pointOnCircleAz = pointOnCircleBz;
			}
		}
		// plot a single pixel
